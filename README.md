...
Standalone packages localize every design dependency the exporter can discover: linked and imported CSS, nested CSS `url(...)` assets, fonts, ordinary and lazy-loaded images, responsive `srcset` sources, video posters, favicons and touch/mask icons, preload design assets, legacy `background` attributes, and SVG image/use references. WordPress discovery metadata, feeds, REST/oEmbed/XML-RPC links, canonical/shortlink/pingback references, obsolete conditional-comment dependencies, unresolved remote assets, external navigation targets, and remote embeds are removed. A final audit stops the export if any nonessential external dependency remains. Google tracking and CentralCRM loaders are the only intentional runtime exceptions and are reported separately.
